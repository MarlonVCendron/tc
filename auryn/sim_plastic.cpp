...
int main(int ac, char* av[]) {
  ...
  float tau_hom = 5.0;  // timescale of homeostatic sliding threshold
  float eta_rel = 0.1;  // relative learning rate
  float kappa = 3.0;    // target rate
  float gamma = 4.0;
  float max_weight = 1.0;
  TripletConnection* con_ee = new TripletConnection(neurons_exc, neurons_exc, weight, sparseness, tau_hom, eta_rel, kappa, max_weight, GLUT);
